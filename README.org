...
Further opinionated configuration imposes restrictions. Downstream modification of template opinions break (as they *have* for me) when the upstream code gets some incompatible commits.
...
- A user may want to make over-writing (supersede) changes and need to synchronize then across a private repository.
...
- If error is encountered, emacs is never launched and exits with error.
