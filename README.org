...
Dependence on either of them forces one to rely on /another later commits/ being flawless in addition to Gnu Emacs' production commits.
...
- Download [[https://gitlab.com/pradyparanjpe/pspmacs/-/raw/master/aid/install-pspmacs.sh?inline=false][install-pspmacs.sh]]
- Run it using a =POSIX= shell
#+begin_src bash :tangle no
  sh install-pspmacs.sh
#+end_src

#+begin_tip
If =curl= is already installed (generally =TRUE=), this may be automated by the terminal command:
#+begin_src bash :tangle no
  curl -fsSL "https://gitlab.com/pradyparanjpe/pspmacs/-/raw/master/aid/install-pspmacs.sh" | sh
#+end_src
#+end_tip
...
#+begin_danger
Do *NOT* run install-script with =sudo= privileges.
The *script* will invoke =sudo= privileges whenever necessary.
#+end_danger
** Copy manually
- Read and understand this README.
- Ensure [[*Requirements][Requirements]] are installed.
...
- Understand and follow the code below
...
- A user may want to make some over-writing changes (supersede) and need to synchronize them across a private /ecosystem/ using a private version-control repository.
  Such changes should be placed in 'exactly the same location' as that of the file to be superseded, but relative to a separate location, =pvt-emacs-directory=.
...
- The value of this variable is learnt from the environment variable =PVT_EMACS_HOME=.
  #+begin_warning
  - When version-controlling =pvt-emacs-directory=, add =local.d= to its =.gitignore=.
...
- Local changes, cache, etc. cannot be synchronized even across a private ecosystem.
  Such changes should be placed in 'exactly the same location' as that of the file to be superseded, but relative to a separate location, =local-emacs-directory=.
...
- Emacs loads configuration in =<user-emacs-directory>/early-init.el= before launching the GUI, and =<user-emacs-directory>/init.el= after.
  ...
  Each file performs its configuration settings and looks for a file located at the same location relative to =pvt-emacs-directory= and then, =local-emacs-directory=.
  If such a file exists, it is loaded.
- Example, the file =<user-emacs-directory>/late/definitions.el= attempts to load files =<pvt-emacs-directory>/late/definitions.el= and =<local-emacs-directory>/late/definitions.el=.
  If they exist, they can overwrite configuration applied by the =<user-emacs-directory>/late/definitions.el=.
#+begin_tip
- =pvt-emacs-directory>/<filename>.{el,org}= and =local-emacs-directory>/<filename>.{el,org}= may be thought as if they were sequential /after-config-hooks/ of =<user-emacs-directory>/<filename>.{el,org}=
...
- Code
  ...
  - The customization file =<local-emacs-directory>/custom.el= will only be loaded after `config.el' only if the value of `pspmacs/load-custom-file' is set to a non-nil.
...
* .gitignore
- Remember to add =.local.d/= to =pvt-emacs-directory/.gitignore=, since it may be used as =local-emacs-directory=.
